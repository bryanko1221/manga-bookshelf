...
const OLD_KEYS=["mangaShelfV241","mangaShelfV24","mangaShelfV23","mangaShelfV22","mangaShelfV2"];
const KEY="mangaShelfV242";let db=load(),filter="all",editingId=null;
function clean(v){return typeof v==="string"?v.trim():v}
function mergeWith19(old){
 const oldBooks=Array.isArray(old?.books)?old.books:[];
 const merged=DEFAULT_BOOKS.map(base=>{
   const found=oldBooks.find(x=>clean(x?.url)?.replace(/\/$/,"")===base.url.replace(/\/$/,"")||clean(x?.title)===base.title);
   if(!found)return{...base};
   return{
     ...base,
     title:clean(found.title)||base.title,
     url:clean(found.url)||base.url,
     category:clean(found.category)||base.category,
     cover:clean(found.cover)||"",
   };
 });
 return{books:merged,records:old?.records||{},favs:old?.favs||{},updates:old?.updates||{}};
}
function load(){
 for(const k of OLD_KEYS){
   try{
     const x=JSON.parse(localStorage.getItem(k)||"null");
     if(x?.books?.length)return mergeWith19(x);
   }catch(e){}
 }
 return{books:DEFAULT_BOOKS,records:{},favs:{},updates:{}};
}
...
function renderContinue(){
 const a=Object.entries(db.records).filter(([id,v])=>v?.last&&book(id)).sort((x,y)=>y[1].last-x[1].last).slice(0,3);
 $("#continueList").innerHTML=a.length?a.map(([id,v])=>{const b=book(id);return `<div class="continueCard"><h3>${esc(b.title)}</h3><p>最近閱讀 · ${new Date(v.last).toLocaleDateString("zh-TW")}</p><a href="${esc(b.url)}" target="_blank" data-read="${id}">繼續閱讀 ↗</a></div>`}).join(""):'<div class="continueCard"><h3>還沒有閱讀紀錄</h3><p>點擊開始閱讀後會出現在這裡。</p></div>';
 document.querySelectorAll("[data-read]").forEach(x=>x.onclick=()=>mark(x.dataset.read));
}
function render(){
 const q=$("#search").value.trim().toLowerCase(),cat=$("#category").value;
 const list=db.books.filter(b=>(!q||b.title.toLowerCase().includes(q))&&(cat==="all"||b.category===cat)&&(filter==="all"||(filter==="fav"&&fav(b.id))||(filter==="new"&&db.updates[b.id])));
 $("#total").textContent=db.books.length;$("#favCount").textContent=Object.values(db.favs).filter(Boolean).length;$("#newCount").textContent=Object.values(db.updates).filter(Boolean).length;$("#count").textContent=list.length+" 部";
 $("#grid").innerHTML=list.map(b=>`<article class="card"><div class="cover">${b.cover?`<img src="${esc(b.cover)}" alt="${esc(b.title)}封面" onerror="this.style.display='none';this.parentElement.insertAdjacentHTML('beforeend','📖')">`:"📖"}${db.updates[b.id]?'<span class="badge">NEW</span>':""}</div><div class="body"><h3>${esc(b.title)}</h3><div class="meta">${b.url.includes("twmanga")?"TWMANGA":"Baozimh"} · ${db.updates[b.id]?"🆕 有更新":"待檢查"}</div><span class="cat">${esc(b.category||"未分類")}</span><div class="actions"><a class="read" href="${esc(b.url)}" target="_blank" data-read="${b.id}">開始閱讀 ↗</a><button class="star ${fav(b.id)?"on":""}" data-fav="${b.id}">${fav(b.id)?"★":"☆"}</button><button class="edit" data-edit="${b.id}">✎</button></div></div></article>`).join("");
 $("#empty").classList.toggle("hidden",!!list.length);
 document.querySelectorAll("[data-fav]").forEach(x=>x.onclick=()=>{db.favs[x.dataset.fav]=!fav(x.dataset.fav);save();render()});
 document.querySelectorAll("[data-read]").forEach(x=>x.onclick=()=>mark(x.dataset.read));
 document.querySelectorAll("[data-edit]").forEach(x=>x.onclick=()=>openEdit(x.dataset.id));
 renderContinue();
}
...
function closeModal(){$(" #modal".trim()).classList.add("hidden")}
...
$("#theme").onclick=()=>{document.documentElement.classList.toggle("dark");localStorage.setItem("mangaTheme242",document.documentElement.classList.contains("dark")?"dark":"light")};
$("#close").onclick=closeModal;$("#cancel").onclick=closeModal;
$("#save").onclick=()=>{
 const b=book(editingId),title=$("#titleInput").value.trim(),url=$("#urlInput").value.trim(),cover=$("#coverInput").value.trim();
 if(!title||!/^https?:\/\//i.test(url)){$("#error").textContent="請輸入漫畫名稱與完整網址。";return}
 if(cover&&!/^https?:\/\//i.test(cover)){$("#error").textContent="封面網址必須以 http:// 或 https:// 開頭。";return}
 b.title=title;b.url=url;b.cover=cover;b.category=$("#catInput").value;save();closeModal();render();
};
if(localStorage.getItem("mangaTheme242")==="dark")document.documentElement.classList.add("dark");render();